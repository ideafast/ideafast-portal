...
export function processDataRow({ subjectIdIndex, visitIdIndex, lineNum, row, parsedHeader, job }: { subjectIdIndex: number, visitIdIndex: number, lineNum: number, row: string[], parsedHeader: IFieldDescriptionObject[], job: IJobEntry<unknown> }) {
    /* pure function */
    const error: string[] = [];
    let colIndex = 0;
