...
export function filterFields(fields: IField[], ontologyTree: IOntologyTree) {
    ...
    const validRouteFields: string[] = ontologyTree.routes.map(el => el.fieldId[0].replace('$', ''));
    fields.forEach(el => {
...
export function findDmField(ontologyTree: IOntologyTree, fields: IField[], key: string) {
    ...
    const field: IField = fields.filter(el => JSON.stringify(['$' + el.fieldId]) === JSON.stringify(node.fieldId))[0];
    if (!field) {
        return null;
    }
    return null;
...
export function tableColumnRender(data, property) {
    if (property.type === 'string') {
        ...
        const input = data.properties[property.title];
        const date = new Date(input);
        const value = isNaN(date.getTime()) ? 'NA' : date.toUTCString();
        return value;
    }
}
