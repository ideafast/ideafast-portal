...
export function standardize(study: IStudy, fields: IFieldEntry[], data, standardizations: IStandardization[], newFields) {
    const records = {};
...
export function dataGrouping(data, format: string) {
    const joinedData = {};
    for (const subjectId of Object.keys(data)) {
        for (const visitId of Object.keys(data[subjectId])) {
...
function insertInObj(obj, levels: string[], lastValue, join: boolean, subjectId: string, visitId: string) {
    let pointer = obj;
...
function preOrderTraversal(node, array: string[]) {
    if (!node) {
        return false;
    }
    if (node.name) {
